...
class AttackHelper {
  ...
  async prepareQuery(force = false) {
    ...
    if (!this.sessionId || force) {
      ...
      this.logHandler.log('【保存session id】：'+ this.sessionId);
  ...
  async queryOrderNum() {
    ...
    let sqlStr = `select DISTINCT num_id,phone_num, is_release, update_time from number_detail where is_delete=0 AND is_release != 1`;// 1 是已完成
    let addSqlParams = [];
    return await query(sqlStr, addSqlParams)
  }
}
